...
for n in [10,15,20,25,30]:
    ...
    avgdist = 0 # Average distance between nodes
    ...
    for i in range(n):
        ...
        x.append(coord[i][0])
        y.append(coord[i][1])
        # Compute pairwise Euclidean distances and accumulate for average distance calculation
        for j in range(n):
            dist[i].append(math.sqrt((coord[i][0] - coord[j][0])**2 + (coord[i][1] - coord[j][1])**2))
            avgdist += dist[i][j]
    # Compute the average distance by dividing the total accumulated distance by the number of node pairs
    avgdist = math.ceil(avgdist/(n*n))

    # ============================================================================
    # INSTANCE PARAMETERS
    # ============================================================================

    # Generate random realese dates: 50% are zero, 50% are random integers between 5 and 2*avgdist
    r = np.zeros(n)
    for i in range(n):
        if random.random() < 0.5:
            r[i] = random.randint(5, 2*avgdist)
    ...
    Q = math.ceil(sum(q)/n*15)

    # Mínimum number of vehicles
    K0 = math.ceil(sum(q)/Q)

    for K in range(K0,K0+3):

        # Tour length limit (Tmax) is set to 70% of the average distance multiplied by the number of nodes divided by the minimum number of vehicles
        Tmax = math.ceil(avgdist*n/K0*0.7)
        ...
        md.setParam(GRB.Param.OutputFlag, 0)  # Display solver progress
        ...
        print("n = ",n)
        print("K = ",K)
        print("Objective = ",md.ObjVal)
        print("Gap = ",md.MIPGap)
        print("Time = ",md.Runtime)